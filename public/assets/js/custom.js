...
//Add Estate
 ...
 //Calculate Management Fee for Rent Input when adding apartment
    ...
    //Agent Invite
    $(document).on('submit','.invite-tenant-to-apartment',function(){
       var _this = $(this);
       var _url = _this.attr("action");
       var _submitBtn = _this.find("button[type='submit']");
                        _submitBtn.html(waitBtn);
   ...
   $(document).on('click','.add-property-to-plan',function(){
      let _this = $(this);
      let newProperty = _this.data("id");
     let parentForm = _this.parents("form");
     let propertyField = parentForm.find("input[name='plan_properties']");
     if (propertyField.val()) {
         let addedProperties = propertyField.val().split(",");
         if (addedProperties.includes(newProperty)) {
             let propertyIndex = addedProperties.indexOf(newProperty);
             addedProperties.splice(propertyIndex,1);
             addedProperties.join(",");
             propertyField.val(addedProperties);
             _this.removeClass("btn-danger").addClass("btn-primary").html('Add to Plan <i class="fa fa-arrow-circle-right"></i>');
         } else {
             addedProperties.push(newProperty);
             addedProperties.join(",");
             propertyField.val(addedProperties);
             _this.removeClass("btn-primary").addClass("btn-danger").html('Remove from Plan <i class="fa fa-trash"></i>');
         }
         return false;
     } else {
         propertyField.val(newProperty);
         _this.removeClass("btn-primary").addClass("btn-danger").html('Remove from Plan <i class="fa fa-trash"></i>');
         return false;
     }
   });

    $(".cannot-add-to-plan").click(function(){
        Lobibox.window({
           title:'Required LPM',
           content: [
               '<p>This package requires property to have LPM before use</p>'
           ],
           height:'100px'
        });
        return false;
